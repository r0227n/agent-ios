...
fn test_mv_basic_file_equivalence() {
    ...
    let _create =
        crate::common::run_idb_file_command(&["push", "/dev/null", &src_path, "--udid", &udid]);

    // Move file with Python idb
    let python_output =
        crate::common::run_idb_file_command(&["mv", &src_path, &dst_path, "--udid", &udid]);
    ...
    let rust_output = crate::common::run_agent_mobile_file_command(&[
        "mv", &src_path, &dst_path, "--udid", &udid,
    ]);

    crate::common::compare_file_command_outputs(&python_output, &rust_output);

    // Cleanup
    let _ = crate::common::run_idb_file_command(&["rm", &dst_path, "--udid", &udid]);
}
...
fn test_mv_multiple_files_equivalence() {
    ...
    let _create1 =
        crate::common::run_idb_file_command(&["push", "/dev/null", &src1, "--udid", &udid]);
    let _create2 =
        crate::common::run_idb_file_command(&["push", "/dev/null", &src2, "--udid", &udid]);
    let _mkdir = crate::common::run_idb_file_command(&["mkdir", &dst_dir, "--udid", &udid]);

    // Move files with Python idb
    let python_output =
        crate::common::run_idb_file_command(&["mv", &src1, &src2, &dst_dir, "--udid", &udid]);
    ...
    // Cleanup
    let _ = crate::common::run_idb_file_command(&["rm", &dst_dir, "--udid", &udid]);
}
...
fn test_mv_with_root_flag_equivalence() {
    ...
    // Cleanup
    let _ = crate::common::run_idb_file_command(&["rm", &dst_path, "--root", "--udid", &udid]);
}
...
fn test_mv_source_not_found_error() {
    ...
    // Both should fail
    let python_output =
        crate::common::run_idb_file_command(&["mv", nonexistent_src, dst_path, "--udid", &udid]);
    let rust_output = crate::common::run_agent_mobile_file_command(&[
        "mv",
        nonexistent_src,
        dst_path,
...
fn test_mv_rename_file_equivalence() {
    ...
    let python_output =
        crate::common::run_idb_file_command(&["mv", &original_name, &new_name, "--udid", &udid]);
    ...
    // Cleanup
    let _ = crate::common::run_idb_file_command(&["rm", &new_name, "--udid", &udid]);
}
