...
### Server-side streaming (log, pull, tail)
...
### Client-side streaming (push, hid)
...
pub async fn run(args: PushArgs) -> CommandResult {
    with_xcuitest(|mut client| async move {
        ...
        // ストリーミング送信
        let chunks = file_data.chunks(4096);
        for chunk in chunks {
            client.push_chunk(chunk).await?;
        }

        client.push_complete().await?;
        Ok(())
    }).await
}
```

### Bidirectional streaming (launch --wait-for)
...
pub async fn run(args: LaunchArgs) -> CommandResult {
    with_xcuitest(|mut client| async move {
        ...
        // 起動リクエスト送信
        tx.send(LaunchRequest::start(&args.bundle_id)).await?;

        // 出力受信
        while let Some(response) = rx.message().await? {
