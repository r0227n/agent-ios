# iOS E2E テスト完全版チェックリスト

このチェックリストは `agent-mobile` CLI の iOS 向け全機能をテストするための完全版です。
...
- [ ] -p ios 単独使用（device boot, device list）
- [ ] -p ios と --session <SESSION> の組み合わせ
- [ ] --headless オプション（device boot のみ）
