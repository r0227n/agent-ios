# Android E2E テスト軽量版チェックリスト

このチェックリストは `agent-mobile` CLI の Android 向け主要機能を素早くテストするための軽量版です。
各カテゴリから代表的な項目を抽出しています。

## 前提条件

- Android エミュレータまたは実機が `adb devices` で認識されていること
- エミュレータが起動済みであること
...
- [ ] app launch com.android.settings --session e2e-test
- [ ] app launch com.android.settings --format json --session e2e-test
- [ ] app terminate com.android.settings --session e2e-test
...
- [ ] tap back --session e2e-test

## UI操作 - テキスト入力

- [ ] fill @eN "test text" --session e2e-test (EditText に対して)
...
1. **要素タイプ**: Android では `EditText` (iOS の `TextField`)、`TextView` (iOS の `StaticText`) など異なる名前を使用
2. **back キー**: Android では `tap back` でバックキーを使用可能
3. **クリップボード**: `device pbcopy/pbpaste` は iOS のみ対応
4. **Settings アプリ**: テスト用には `com.android.settings` が便利
