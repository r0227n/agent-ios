...
use crate::proto::idb::{AddMediaRequest, Payload};
...
impl IdbClient {
    ...
    pub async fn photos_clear(&mut self) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        let request = tonic::Request::new(crate::proto::idb::PhotosClearRequest {});
    ...
    pub async fn record_video(
        ...
        use crate::proto::idb::record_request::{Control, Start, Stop};
        use crate::proto::idb::RecordRequest;
    ...
    pub async fn video_stream(
        ...
        fps: Option<u64>,
        format: crate::proto::idb::video_stream_request::Format,
        compression_quality: f64,
        scale_factor: f64,
        mut stop_rx: watch::Receiver<bool>,
    ) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        use crate::proto::idb::video_stream_request::{Control, Start, Stop};
        use crate::proto::idb::VideoStreamRequest;
        ...
        loop {
            tokio::select! {
                ...
                msg = stream.message() => {
                    match msg {
                        Ok(Some(response)) => {
                            if let Some(crate::proto::idb::video_stream_response::Output::Payload(payload)) = response.output {
